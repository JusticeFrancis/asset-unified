...
import { derivePropertyFields, propertyDetail } from "@/admin/lib/server/property-serializers";
import { recordActivity } from "@/admin/lib/server/activity";
import { beginPropertyProvisioning } from "@/admin/lib/server/integrations";
import { removePropertyClientRecord, syncPropertyToClientRecord } from "@/admin/lib/server/client-record-sync";
...
export async function PUT(request: NextRequest, context: { params: Promise<{ propertyId: string }> }) {
  try {
    ...
    await property.save();
    if (property.status === "active") await syncPropertyToClientRecord(property);
...
export async function DELETE(request: NextRequest, context: { params: Promise<{ propertyId: string }> }) {
  try {
    ...
    const property = await Property.findById(propertyId);
    if (!property) throw new HttpError(404, "NOT_FOUND", "Property not found.");
    await removePropertyClientRecord(property);
