...
export async function POST(request: NextRequest) {
  try {
    ...
    const user = await User.create({ fullName: body.fullName.trim(), email: body.email.trim().toLowerCase(), phoneNumber: body.phone || "", country: body.country || "", status: body.status || "active", kycStatus: body.kycStatus || "not_started", accountType: body.accountType || "investor", acceptedTermsAt: body.acceptedTermsAt || null, emailVerifiedAt: body.acceptedTermsAt || null, metadata: body.metadata || {} });
