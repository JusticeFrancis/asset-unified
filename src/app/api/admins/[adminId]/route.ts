...
import { Admin, AdminInvite, AdminSession } from "@/admin/models";
...
import {
  ...
  permissionsFor,
  type AdminRoleName,
} from "@/admin/lib/server/constants";
...
export async function GET(request: NextRequest, context: { params: Promise<{ adminId: string }> }) {
  try {
    ...
    const invite = await AdminInvite.findOne({ adminId }).sort({ createdAt: -1 }).lean() as { expiresAt?: Date } | null;
    return ok({
      admin: {
