...
import { GovernanceVoteConfirmModal } from "@/app/dashboard/governance/components/governance-vote-confirm-modal";
import type {
  GovernanceProposalRecord,
  GovernanceVoteChoice,
} from "@/app/dashboard/governance/data/governance-records";
import { appQueryKeys } from "@/lib/api/queries/app";
import { voteGovernanceProposal } from "@/lib/api/requests/app";
...
export function GovernanceProposalDetailView({
  ...
  const onConfirmVote = async () => {
    const response = await voteGovernanceProposal(proposal.slug, choice);
    const vote = response.vote;
    setVoteSummary({
      choice: vote.choice,
      votes: Number(vote.weight || proposal.userVotingPower),
      dateLabel: new Date(vote.createdAt).toLocaleDateString("en-GB", {
        day: "numeric",
        month: "short",
        year: "numeric",
      }),
    ...
    setHasVoted(true);
    await Promise.all([
      queryClient.invalidateQueries({ queryKey: appQueryKeys.proposal(proposal.slug) }),
      queryClient.invalidateQueries({ queryKey: ["app", "governance"] }),
    ]);
  };
  ...
  const voteOption = (
    ...
    <button
      ...
      onClick={onSelect}
    >
      <span className="mt-0.5 shrink-0 text-[#5c60cc]">
        {selected ? (
          <CircleDot aria-hidden className="size-[14px]" strokeWidth={1.75} />
        ) : (
          <Circle aria-hidden className="size-[14px]" strokeWidth={1.75} />
        )}
      </span>
      <span className="flex min-w-0 flex-col gap-1 text-[12px] leading-normal">
        <span className="font-normal text-[#050a0e]">{choiceLabel(value)}</span>
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1118px] flex-col gap-5 lg:flex-row lg:items-start">
      <div className="flex min-w-0 flex-1 flex-col gap-5">
        <div className="rounded-[20px] bg-white p-4 shadow-[0_1px_4px_rgba(12,12,13,0.05)] sm:p-6">
        ...
        <div className="rounded-[20px] bg-white p-4 shadow-[0_1px_4px_rgba(12,12,13,0.05)] sm:p-6">
          ...
          <div className="mt-4 flex items-center gap-2 rounded-lg bg-[#cff7d3] px-3 py-2 sm:px-6">
            <Check
              aria-hidden
        ...
        {showVotePanel ? (
          <div className="rounded-[20px] bg-white p-4 shadow-[0_1px_4px_rgba(12,12,13,0.05)] sm:p-6">
            ...
            <div className="mt-2 flex flex-col gap-2">
              {voteOption("for", choice === "for", () => setChoice("for"))}
              {voteOption("against", choice === "against", () =>
                setChoice("against"),
              )}
              {voteOption("abstain", choice === "abstain", () =>
                setChoice("abstain"),
              )}
            </div>
