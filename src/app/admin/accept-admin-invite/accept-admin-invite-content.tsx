...
import { apiRequest } from "@/admin/lib/api/client";
...
export function AcceptAdminInviteContent(){
  ...
  return <main className="flex min-h-screen items-center justify-center bg-[#edf4f8] px-4 py-8"><div className="w-full max-w-[430px] rounded-[20px] bg-white p-6 shadow-[0_1px_4px_rgba(12,12,13,0.05)] sm:p-8">
    ...
    {loading?<p className="text-center text-[12px] text-[#919191]">Validating invitation…</p>:done?<div className="space-y-4 text-center"><p className="rounded-[12px] bg-[#edf9f2] p-4 text-[13px] text-[#16834d]">Invitation accepted successfully.</p><Link href="/admin/sign-in" className="inline-flex h-10 items-center justify-center rounded-[12px] bg-[#5c60cc] px-6 text-[12px] font-medium text-white">Sign in</Link></div>:error&&!invite?<div className="space-y-4 text-center"><p className="rounded-[12px] border border-[#fecaca] bg-[#fef2f2] p-4 text-[12px] text-[#b91c1c]">{error}</p><Link href="/admin/sign-in" className="text-[12px] font-medium text-[#5c60cc]">Return to sign in</Link></div>:<form onSubmit={submit} className="space-y-4">
