...
import type { PropertyType } from "@/app/admin/(app)/property-management/property-management-data";
import { PropertyRowActionsMenu } from "@/app/admin/components/property-row-actions-menu";
import { AdminDataTablePagination } from "@/app/admin/components/admin-table-pagination";
import { ADMIN_ASSETS } from "@/app/admin/components/admin-assets";
import { ScrollableTabRail } from "@/app/admin/components/scrollable-tab-rail";
...
} from "@/admin/lib/property-wizard/mappers";
import { propertyManagementDetailHref } from "@/admin/lib/property-wizard/paths";
import { useScopedProperties } from "@/admin/lib/property-wizard/use-scoped-property-api";
import { canEditOrgProperties } from "@/admin/lib/organization-permissions";
import { useOrganizationAuth } from "@/admin/contexts/organization-auth-provider";
import { canUpdateProperties } from "@/admin/lib/admin-permissions";
      ...
      cell: ({ row }) => (
        <div className="flex min-w-0 items-start gap-2">
          ...
          <div className="min-w-0 leading-snug">
            <Link
              className="break-words text-[12px] font-normal text-[#050A0E] hover:text-[#5c60cc]"
              href={propertyManagementDetailHref(paths, row.original.id)}
            >
    ...
    <Card className="w-full min-w-0 rounded-[20px] border-0 p-4 shadow-sm sm:p-6">
      ...
      {isLoading ? (
        ...
        <DataTable
          ...
          onRowClick={(row) => router.push(propertyManagementDetailHref(paths, row.id))}
          renderFooter={(table) => (
