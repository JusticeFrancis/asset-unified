...
import { HttpError } from "@/admin/lib/server/http";
import { hashToken, randomToken } from "@/admin/lib/server/crypto";
import { permissionsFor, type AdminRoleName, type Permission } from "@/admin/lib/server/constants";
import { Admin, AdminSession } from "@/admin/models";
...
export async function ensureBootstrapSuperAdmin() {
  ...
  const activeSuperAdmin = await Admin.exists({ role: "super_admin", status: "active" });
  const configuredAdmin = await Admin.findOne({ email }).select("+passwordHash");
  const shouldForceReset = process.env.SUPER_ADMIN_FORCE_RESET === "true";

  if (configuredAdmin) {
    if (!activeSuperAdmin || shouldForceReset) {
      configuredAdmin.fullName = process.env.SUPER_ADMIN_NAME || configuredAdmin.fullName || "Super Admin";
      configuredAdmin.passwordHash = await hashPassword(password);
      configuredAdmin.role = "super_admin";
      configuredAdmin.status = "active";
      configuredAdmin.inviteAcceptedAt = configuredAdmin.inviteAcceptedAt || new Date();
