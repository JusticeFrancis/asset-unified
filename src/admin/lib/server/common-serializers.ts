export function userJson(item: any) {
  return { id: String(item._id), fullName: item.fullName, email: item.email, phone: item.phone || item.phoneNumber || "", avatarUrl: item.avatarUrl || "", country: item.country || "", status: item.status, kycStatus: item.kycStatus, accountType: item.accountType || item.metadata?.accountType || "investor", acceptedTermsAt: item.acceptedTermsAt || item.emailVerifiedAt || null, lastLoginAt: item.lastLoginAt, metadata: item.metadata || {}, createdAt: item.createdAt, updatedAt: item.updatedAt };
}
