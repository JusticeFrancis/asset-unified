import { Admin, Notification } from "@/admin/models";
import { sendMail, escapeHtml } from "@/admin/lib/server/mailer";
import { UserModel as User } from "@/lib/server/models";
import { deliverClientInAppNotification } from "@/admin/lib/server/client-record-sync";
...
export async function deliverNotification(notification: any) {
  ...
  const inAppRecipients = await deliverClientInAppNotification(notification);
  if (channels.includes("email")) {
    if (notification.audience === "Investors") {
      const users = await User.find({ status: "active" }).select("email").lean();
  ...
  notification.recipientsCount = Math.max(emailRecipients.length, inAppRecipients);
