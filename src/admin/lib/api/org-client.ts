...
import { resolveApiError } from "@/admin/lib/api/parse-error";
import { ApiError } from "@/admin/lib/api/types";
import { normalizeRequestError } from "@/admin/lib/api/user-facing-error";
import { getActiveOrganizationId } from "@/admin/lib/auth/org-session";
import { getOrgAccessToken } from "@/admin/lib/auth/org-tokens";
...
export function getApiBaseUrl() {
  const configured = process.env.NEXT_PUBLIC_ADMIN_API_URL?.replace(/\/+$/, "");
